...
Check make_lemmas.
Check test_already_exists.

Fixpoint get_contract_name (s:string) :=
match s with
| "" => ""
| String ("_"%char) t => ""
| String h t => String h ( get_contract_name t )
end.

Compute ( get_contract_name "DePoolContract" ).

Fixpoint make_out(l:list string)(sl : list (list string)) :=
match sl with 
| nil => nil
| h :: t => let q := headS "" h in
            let w := get_contract_name q in
            if ( test_already_exists w l )
            then 
                 let e := tail h in
                 e :: make_out l t

            else let e := tail h in
 ("(**" :: ("./Proofs/"++w++"Proof.v")%string :: "*)" :: header :: e ) :: make_out (w::l) t
end.
