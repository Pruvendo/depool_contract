...
| REQUIRE, h :: t => 
     (* acc ++ *) "require" :: "(" :: "tt" :: (pull_out NOTHING [] t)
...
| IFF, h :: t => 
     (* acc ++ *) "If" :: "(" :: "tt" :: (pull_out NOTHING [] t)

| RETURN, h :: t => 
     (* acc ++ *) "return!" :: "(" :: "tt" :: ")" :: (pull_out NOTHING [] t)
...
| "return!" :: "(" :: h :: ")" :: t => 
       ...
       then "(" :: "<-v" :: h :: ")" :: ">>=" :: "->l" :: "tt" :: "in"  :: newline ::
         tab :: "return!" :: "(" :: "tt" :: ")" :: work_with_return t
       else
       if(have_it_symbol "164"%char h)   (* Ф *)
       then "(" :: "<-f" :: h :: t ++ ")" :: ">>=" :: "->l" :: "tt" :: "in"  :: newline ::
         tab :: "return!" :: "(" :: "tt" :: ")" :: work_with_return t
       else
...
let list_of_ids := get_list_of_ids jj in 
let exch := get_exchs_of_ids jj in


                            let ff := (* map work_with_operations *) (map clear_string_easy exch) in
                            ...
                            *) 
let ll  := map (work_with_let false [])                 ff in

let ll := map (work_with_structConstr false [] (map snd struct_list) ) ll in

                              (* let xx := map work_with_mmpp ll in *)

let ii' := map func_name_to_inside                      ll in 

                              let qq'' := map semicolon_plus_newstring ff in  
...
let ee := map work_with_IProxy qq' in

                                         (* let rr := map work_with_return ee in  *)

( concat_with newline  ( map ( concat_with " " )     qq''   ) ).
