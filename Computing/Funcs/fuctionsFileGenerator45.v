...
↑11  (modify (fun r => setEventList (xListCons e (VMState_ι_events r)) r) >> 
	  return! I ).
...
let dd := map ( get_to_sol numbers_contract_vars ) dd in                         (* -- ^^ -- *)
...
let dd := map all_end_prepare dd in        (* let dd := [ headS [] dd ] in *) 


( header ++ concat_with newline  ( map ( concat_with " " ) dd  ) ++ footer )%string. 
