...
match sl with
| key_word :: name :: nil => 
(prefix'++"·"++name)%string :: ":" :: (sol_types2coq_mini_types key_word) :: ";" :: newline :: nil
| key_word :: modif :: name :: nil => 
(prefix'++"·"++name)%string :: ":" :: (sol_types2coq_mini_types key_word) :: ";" :: newline :: nil
| "mapping" :: "(" :: type1 :: "=>" :: type2 :: ")" :: name :: nil =>
(prefix'++"·"++name)%string :: ":" :: "Arr" :: (sol_types2coq_mini_types type1) ::
...
let sl := [("{"++newline)%string] ++ (map (concat_with  " ")  sl) ++ [(newline++"}."++newline)%string] ++ nil in sl.
...
| h :: t => let struct_name := 
     (contract_name++"·"++h)%string in (*||||||||||||||||||||||||||||||||||||||||||||||||||||||||*)
         ...
         "Arguments " ++ (struct_name ++ "C")%string ++
                " [ " ++ (concat_with " " typelist) ++" ] " ++ "." ++ newline)%string]  
end.
