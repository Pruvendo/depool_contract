...
(tab++prefix'++"·"++name)%string :: ":" :: (sol_types2coq_mini_types key_word) :: ";" :: 
...
| "}.
" :: t  => list_pass t
| h :: t => ("default" ++ " " ++ (list_pass t))%string
end.
...
| h :: t => let x := make_struct_interior struct_name t in
        ...
        let natural_type_list := (concat_with " " (map cot_mini_types2coq_types typelist)) in
         ...
         ("Instance" :: (struct_name++"_default")%string :: ":" :: "XDefault" :: struct_name ::
          ":=" :: "{" :: newline :: 
          tab :: "default" :: ":=" :: ( struct_name ++ "C")%string :: (list_pass x) :: "}" :: ["."])   
end.

Definition get_struct (contract_name:string) (sl:list string) :=
match sl with                                               (* get_type_list *)
| nil => nil
| h :: t => let struct_name := (contract_name++"·"++h)%string in 
...
| nil => nil
| key_word :: name :: nil => 
(tab++prefix'++"·"++name)%string :: ":" :: (sol_types2coq_mini_types key_word) :: ";" :: nil
...
map (make_elses' contract_name) z.


(* make_elses' contract_name ( make_list_level_0 false 0 sl). *)


Compute ( make_elses "-" (* (make_list_level_0 false 0 *) ["uint";"qwerty";";";     
 "bool";"payable";"asdfghj";";"
;"mapping";"(";"uint64";"=>";"address";")"; "zxcvbnm";";";
"";"uint";"payable";"xru";";";"uint16";"d";";";"uint256";"qqq";";";"uint16";
"ddd";";";"";"";"";"";"";"";"";""]).
...
let structs   :=  [ctrt ; contract_name] :: str in 
...
let listlist := sl in (* неслипшийся списокЪ *)

let sl := List.concat sl in

let all := make_contracts sl (* listlist *) in
...
contract OwnerBase {
    ...
    Owner m_owner;
    constructor(address payable poolOwnerAddr) internal {
    ...
    function withdrawOwnerReward(uint64 amount) public payable onlyOwnerContract {
