...
match sl with
| nil => nil
| ":" :: h :: h1 :: h2 :: t => let q := get_non_standard_type tl h in
                     if ( q =? "" ) 
                     then 
...
match sl with
| nil => "" (* nil *)
| "(@" :: name :: t => name (* :: find_brace_dog' t *)
| h :: t => find_brace_dog' t
end.
Definition find_brace_dog (sl:list string ) :=
let q := find_brace_interior (-1) sl in
let w := tail q in
let e := last_delete w in find_brace_dog' e.
Fixpoint adding_parameters(tl:list(string * list string))(sl:list string):=
match sl with
| nil => nil
| "
Record" :: name :: t => 
          let qq := find_brace_interior (0) (tail (tail t)) in (* find in record body *)
          let q := find_brace_interior (-1) t in (* find in parameters *)
          let w := tail q in
          let e := last_delete w in    
          let y := ( find_brace_dog qq ) in 
          if ( y =? "" ) 
          then newline :: "Record1" :: name :: adding_parameters tl t
          else
              let r := find_typelist tl y in
(*               let count := ( List.length r ) in
              if ( Nat.eqb count 0 )
              then *) 
              newline :: "Record" :: name :: "{2" :: ( list_of_uniq_types e r ) ++ "}" :: adding_parameters tl t
