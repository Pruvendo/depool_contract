...
| nil => nil
| name :: "=" :: x :: ";" :: t => 
      (match (who name),(who x) with
      | NUMBER, _ => nil
      | LOCAL, NUMBER => "( <-c " :: x  :: ")":: ">>=" :: "->l" :: name :: ["in"]
      | LOCAL, LOCAL  => "( <-l " :: x  :: ")":: ">>=" :: "->l" :: name :: ["in"]
      | LOCAL, VAR    => "( <-v " :: x  :: ")":: ">>=" :: "->l" :: name :: ["in"]
      | LOCAL, FUNf   => "( <-f " :: x  :: ")":: ">>=" :: "->l" :: name :: ["in"]
      | LOCAL, FUNdo   => "do" :: name :: "← ↓ (" :: x :: ")" :: [";;??"]
      | VAR,   NUMBER => "( <-c " :: x  :: ")":: ">>=" :: "->v" :: name :: ["in"]
      | VAR,   LOCAL  => "( <-l " :: x  :: ")":: ">>=" :: "->v" :: name :: ["in"]
      | VAR,   VAR    => "( <-v " :: x  :: ")":: ">>=" :: "->v" :: name :: ["in"]
      | VAR,   FUNf   => "( <-f " :: x  :: ")":: ">>=" :: "->v" :: name :: ["in"]
      | VAR, FUNdo   => let tmp_name:=(x++"_Т_")%string in
                            "do" :: tmp_name :: "← ↓ (" :: x :: ")" :: ";;??" ::
                            "( <-l" :: tmp_name :: ")" :: ">>=" :: "->v" :: name :: ["in"]
      | _, _ => nil
      end ) :: [newline] :: (get_body t)
| typ :: name :: "=" :: x :: ";" :: t => 
      (match (who name),(who x) with
      | NUMBER, _ => nil
      | LOCAL, NUMBER => "( <-c " :: x  :: ")":: ">>=" :: "->l" :: name :: ["in"]
...
(* let functions :=                                               (* structs ++ *) 
                 (make_functions contract_name x) in  
