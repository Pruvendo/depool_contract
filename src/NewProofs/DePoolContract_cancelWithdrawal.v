...
Module DePoolContract_Ф_cancelWithdrawal (dc : DePoolConstsTypesSig XTypesSig StateMonadSig).
...
Lemma DePoolContract_Ф_cancelWithdrawal_exec : forall (l : Ledger),
...
let newParticipant := {$ participant with (DePoolLib_ι_Participant_ι_reinvest, true);
                                          (DePoolLib_ι_Participant_ι_withdrawValue, 0) $} in
...
exec_state (↓ DePoolContract_Ф_cancelWithdrawal) l =
...
Lemma DePoolContract_Ф_cancelWithdrawal'_eval : forall (l : Ledger),
let sender := eval_state msg_sender l in
let isInternalMessage : bool := negb (sender =? 0) in
let isPoolClosed : bool := eval_state (↑ε12 DePoolContract_ι_m_poolClosed) l in
let optParticipant := eval_state (↓ ParticipantBase_Ф_fetchParticipant sender) l in
...
eval_state (↓ DePoolContract_Ф_cancelWithdrawal') l =
...
Opaque DePoolContract_Ф_cancelWithdrawal'.

Lemma DePoolContract_Ф_cancelWithdrawal_eval : forall (l : Ledger),
let sender := eval_state msg_sender l in
let isInternalMessage : bool := negb (sender =? 0) in
let isPoolClosed : bool := eval_state (↑ε12 DePoolContract_ι_m_poolClosed) l in
let optParticipant := eval_state (↓ ParticipantBase_Ф_fetchParticipant sender) l in
...
Proof.
  ...
  assert (eval_state (↓ DePoolContract_Ф_cancelWithdrawal ) l = 
  xErrorMapDefaultF (xValue ∘ fromValueValue) (eval_state (↓ DePoolContract_Ф_cancelWithdrawal' ) l) xError ).
  unfold DePoolContract_Ф_cancelWithdrawal.
  unfold callEmbeddedStateAdj.
  remember (DePoolContract_Ф_cancelWithdrawal' ).
  ...
  (**********************)
  rewrite H.
  rewrite DePoolContract_Ф_cancelWithdrawal'_eval.
  compute.
  repeat destructIf; auto.
Qed. 

End DePoolContract_Ф_cancelWithdrawal.