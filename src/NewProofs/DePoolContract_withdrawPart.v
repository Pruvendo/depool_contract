...
Module DePoolContract_Ф_withdrawPart (dc : DePoolConstsTypesSig XTypesSig StateMonadSig).
...
Lemma DePoolContract_Ф_withdrawPart'_eval : forall (l : Ledger)(Л_withdrawValue : XInteger64),
let sender := eval_state msg_sender l in
let isInternalMessage := negb (sender =? 0) in
let isPoolClosed : bool := eval_state (↑ε12 DePoolContract_ι_m_poolClosed) l in
let optParticipant := eval_state (↓ ParticipantBase_Ф_fetchParticipant sender) l in
...
Proof.
  ...
  assert (eval_state (↓ DePoolContract_Ф_withdrawPart Л_withdrawValue) l = 
  xErrorMapDefaultF (xValue ∘ fromValueValue) (eval_state (↓ DePoolContract_Ф_withdrawPart' Л_withdrawValue) l) xError ).
  unfold DePoolContract_Ф_withdrawPart.
  unfold callEmbeddedStateAdj.
  remember (DePoolContract_Ф_withdrawPart' Л_withdrawValue).
  ...
  (**********************)
  rewrite H.
  rewrite DePoolContract_Ф_withdrawPart'_eval.
  compute.
  repeat destructIf; auto.
Qed.

End DePoolContract_Ф_withdrawPart.