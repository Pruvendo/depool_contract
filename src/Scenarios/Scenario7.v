...
Module Scenario7  (dc : DePoolConstsTypesSig XTypesSig StateMonadSig).
...
Lemma Scenario_StakeWin_ParticipantVestingValidatorOrdinary: forall 
...
let reward_all := msg_value_onSuccessToRecoverStake + DePoolLib_ι_PROXY_FEE - stake/2 - stakeV - (DePool_ι_RET_OR_REINV_FEE +  2 * DePool_ι_RET_OR_REINV_FEE) in
let rewards := reward_all * participantRewardFraction / 100  in
let stakeSum := stake/2 in 
let stakeSumV := stakeV in 
let reward := stakeSum * rewards / (stake/2 +stakeV)  in
let rewardV := stakeSum * rewards / (stake/2 +stakeV)  in
