...
Module Scenario5  (dc : DePoolConstsTypesSig XTypesSig StateMonadSig).
...
Lemma Scenario_OrdinaryStakeWin_byValidatorAndParticipant: forall 
...
(negb (tvm_hash ((NetParams_toWaitingReward ->> NetParams_ι_prevValidatorData)) =? tvm_hash ((NetParams_round1 ->> NetParams_ι_curValidatorData)))))%bool= true ->      (* условие может быть пройдено иначе*)
...
(msg_value_onSuccessToRecoverStake + DePoolLib_ι_PROXY_FEE >=? stakeP+ stakeV) = true ->

let reward_all := msg_value_onSuccessToRecoverStake + DePoolLib_ι_PROXY_FEE - stakeP - stakeV  - (DePool_ι_RET_OR_REINV_FEE +  2 * DePool_ι_RET_OR_REINV_FEE) in
let rewards := reward_all * participantRewardFraction / 100  in
let stakeSum := stakeP in 
let stakeSumV := stakeV in 
let reward := stakeSum * rewards / (stakeP +stakeV)  in
let rewardV := stakeSumV * rewards / (stakeP +stakeV)  in
