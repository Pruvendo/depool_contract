(***************    Work with rest stuff of contracts   *********    ********)
...
| false, 0, _ :: "(" :: ")" :: "external" :: _ :: t => make_list_level_0 f n t
...
(* Working of contract's variables ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc*)
...
(* The first line is a name of contract *)  
...
(* Workin of contract's functions ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc*)
...
(* | "interface" :: name :: t => functions_list' name t  *)
| "function" :: name :: t =>  (name, (s++"_Ф_"++name)%string) :: functions_list' s t
| h :: t => functions_list' s t
end.
(* delete repetitive funcnames *)
Fixpoint delete_repetitive_funcnames(l : list string) (sl :list ( string * string)):=
match sl with
| nil => nil
| h :: t => let q := fst h in
            if(test_already_exists q l) 
            then delete_repetitive_funcnames l t
            else h :: delete_repetitive_funcnames ( q :: l ) t
end.
Definition functions_list(s:string) (sl:list string) :=
let q := functions_list' "" sl in q.
(* let w := delete_repetitive_funcnames [] q in w. *)
...
Fixpoint get_list_executing_functions(sl:list string) :=
match sl with
| nil => nil
| h :: t => if (have_it_symbol "164"%char h) (* Ф *)
            then h :: get_list_executing_functions t
            else get_list_executing_functions t
end.
(* moving functions to end *)
Fixpoint func_exchange (acc : list ( list string )) (sl:list string):=
match acc with
| nil => nil
| h :: t => let q := headS "" ( tail h ) in
            let w := headS "" ( tail sl ) in
            if ( q =? w ) then ( func_exchange t sl ) ++ [h]
                          else h :: func_exchange t sl
end.
Fixpoint rigth_order_funcs( already_func_list : list string )
                          ( acc : list ( list string ) )
                          ( sl : list ( list string ) ) :=
match sl with
| nil => acc
| h :: t => let q := headS "" (tail h) in (* func name *)
            let w := tail (tail h) in     (* body *)
            let e := get_list_executing_functions w in
            if ( test_not_already_exists_for_list e already_func_list )
            then (* There are nothing *)
                 let acc := func_exchange acc h in
                 rigth_order_funcs already_func_list acc t
            else 
                 let already_func_list := already_func_list ++ h in
                 rigth_order_funcs already_func_list acc t
end.


(* Functions without running functions was contain into top of list *)
...
let ll := (List.length t) in
let f := tt ++ (function_order'' ( ll * ll  ) [] t) in
...
Fixpoint find_coq_name2(s : string)(tl:list (string * string)) := 
match tl with
| nil => ""
| h :: t => if(s =? (snd h)) then snd h
                             else find_coq_name s t
end.
(* rename from solidity mode to coq mode *)
Fixpoint rename_sol2coq1(s:string)(tl:list (string * string))(sl:list string) :=
match sl with
| nil => nil
| "contract" :: name :: t => rename_sol2coq1 name tl t

| h :: t => let q := find_coq_name2 (s++"_Ф_"++h)%string tl in 
            if(q =? "")
            then (
                 let w := find_coq_name h tl in 
                 if(w =? "") 
                 then h :: (rename_sol2coq1 s tl t)(* h isnt function *)
                 else w :: (rename_sol2coq1 s tl t)(* if the func isnt into current contract*)
                 )
            else (s++"_Ф_"++h)%string :: (rename_sol2coq1 s tl t)(* if the func is into current contract*)
end.
...
match sl with
| nil => nil
| "true" :: t => "$" :: "xBoolTrue" :: rename_numbers t
| "false" :: t => "$" :: "xBoolFalse" :: rename_numbers t
...
Fixpoint numering_not_empty_contracts(n:nat)(sl:list string) :=
match sl with
| nil => nil
| "contract" :: name :: t => let q := find_brace_interior (-1) t in 
                             let w := last_delete ( List.tail q ) in
                             let e := make_list_level_0 false 0 w in
                                let r := List.length e in
                             if ( Nat.eqb ( r ) 0 ) 
                             then numering_not_empty_contracts n t
                             else ( name , ( "↑"++( writeNat n ) )%string ) ::
                                  numering_not_empty_contracts (n+1) t
| h :: t => numering_not_empty_contracts n t
end.



Fixpoint get_number_contract_vars(name:string)(sl:list string) :=
match sl with
| nil => nil (*mapping(uint32 => Round) m_pendingRounds;*)
| "mapping" :: "(" :: _ :: "=>" :: h1 :: ")" :: h2 :: ";" :: t => 
                   let q := first2 h2 in
                   if ( q =? "m_" )
                   then 
                        let w := add_exist_types ["address";"Address"] in
                        if ( test_already_exists h1 w )
                        then 
                             get_number_contract_vars name t
                        else 
   (* ( h2 , (name++"_ι_"++h1)%string , "↑↑" ) :: *)
   ( h2 , "↑↑" ) ::
                            get_number_contract_vars name t
                   else
                        get_number_contract_vars name t

| h1 :: h2 :: ";" :: t => let q := first2 h2 in
                   if ( q =? "m_" )
                   then 
                        let w := add_exist_types ["address";"Address"] in
                        if ( test_already_exists h1 w )
        ...
        ( h2 , "↑↑" ) ::      
                        ...
                        get_number_contract_vars name t
| h :: t => get_number_contract_vars name t
end. 

(* For difficult types *)
Fixpoint numering_contract_vars'(sl:list string) :=
match sl with
| nil => nil
| "contract" :: name :: t => let q := find_brace_interior (-1) t in 
                             let w := last_delete ( List.tail q ) in
                             ...
                             if ( Nat.eqb r 0 ) 
                             then 
                                  numering_contract_vars' t
                             else 
                                 ( get_number_contract_vars name e ) ++
                                  numering_contract_vars' t
| h :: t => numering_contract_vars' t
end.
Fixpoint plust_tuple (n:nat) (tl:list(string * string)):=
match tl with
| nil => nil
| ( a , h2 ) :: t => ( a , (h2++(writeNat n) )%string ) :: plust_tuple (n+1) t
end.
Definition numering_contract_vars(sl:list string) :=
let q := numering_contract_vars' sl in
plust_tuple 1 q.
